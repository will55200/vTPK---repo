# This script will set the map extent to your desired AOI(s), find the associated vector tile index and process the vector tile package at user defined scales.
# If there are multiple AOIs the script will loop through them and calculate the time it takes to process each AOI and write that to a text log.
# The AOI and the vector tile index names MUST MATCH.
# In the promts DO NOT USE any " " or , 
...
def main():
    ...
    gdb_path = input("Enter the path to the geodatabase containing index polygons: ")
    ...
    for index, fc in enumerate(feature_classes, start=1):
        ...
        try:
            ...
            if arcpy.Exists(fc_path):
                print(f"Feature class '{fc}' exists in the index.gdb.")
